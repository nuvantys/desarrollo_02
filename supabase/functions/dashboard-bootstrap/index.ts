...
async function readSnapshotAsset(filename: string) {
  const admin = createAdminClient();
  const { data, error } = await admin.rpc("get_snapshot_asset", { p_filename: filename });
  if (error) {
    throw new Error(`No fue posible leer ${filename} desde el snapshot privado. ${error.message}`);
  }
  if (!data) {
    throw new Error(`El archivo ${filename} aun no existe en app.snapshot_assets.`);
  }
  return data;
}
...
function selectWorkflowRun(payload: Record<string, unknown>) {
  const runs = Array.isArray(payload.workflow_runs) ? payload.workflow_runs as Array<Record<string, unknown>> : [];
  return runs[0] ?? null;
}

async function fetchJobsPayload(runId: string, githubToken: string, githubOwner: string, githubRepo: string) {
  const jobsResponse = await fetch(
    `https://api.github.com/repos/${githubOwner}/${githubRepo}/actions/runs/${runId}/jobs?per_page=100`,
    {
      headers: {
        Accept: "application/vnd.github+json",
        Authorization: `Bearer ${githubToken}`,
        "User-Agent": "dashboard-bootstrap",
      },
    },
  );
  if (!jobsResponse.ok) {
  ...
  return await jobsResponse.json() as Record<string, unknown>;
}
...
async function fetchRuntimeState() {
  try {
    ...
    const run = selectWorkflowRun(payload);
    if (!run?.id) {
      return { current_job: null, last_job: null };
    }

    const jobsPayload = await fetchJobsPayload(String(run.id), githubToken, githubOwner, githubRepo);
...
Deno.serve(async (request) => {
  ...
  try {
    const [manifest, overview, technical, runtime] = await Promise.all([
      ...
      readSnapshotAsset("technical.json"),
      fetchRuntimeState(),
    ]);
    ...
    return json({
      ...
      technical,
      runtime,
      bootstrap_generated_at: technical?.generated_at || manifest?.generated_at || null,
      run_id: technical?.run_id || manifest?.run_id || null,
      coverage_min: technical?.coverage_min || manifest?.coverage_min || null,
